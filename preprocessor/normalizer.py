...
class Normalizer:
    ...
    def delete_invalid_character(self, in_string: str):
        result_list = [
            char if char not in DELETE_CHARACTER else '' for char in in_string
        ]
    ...
    def correct_number_spacing(self, in_string: str):
        normal_string = [(SPACE if char in PERSIAN_NUMBERS and prev_char not in PERSIAN_NUMBERS else "") + char for prev_char, char in zip([''] + list(in_string), in_string)]
